 ...
 */
function generatePuyoPools(seed: number, codes = [1, 2, 3, 4, 5]): [number[], number[], number[]] {
  const RNG = new TsuRNG(seed);

  // Initialize pools
  const pool1: number[] = Array(256);
  const pool2: number[] = Array(256);
  const pool3: number[] = Array(256);
  for (let i = 0; i < 256; i++) {
...
export default generatePuyoPools;
